...
/* UserMenu — supports account-switch destinations passed via switches=[{label,to}]. */
function UserMenu({ user, role, onLogout, switches = [] }) {
      ...
      {open && (
        <div style={{ position: "absolute", top: "calc(100% + 8px)", right: 0, minWidth: 220, background: "#fff", borderRadius: 12, boxShadow: "0 10px 28px -8px rgba(21,42,36,0.18), 0 4px 8px -4px rgba(21,42,36,0.08)", border: "1px solid rgba(21,42,36,0.08)", padding: 6, zIndex: 100 }}>
          ...
          {switches && switches.length > 0 && (
            <React.Fragment>
              <div style={{ height: 1, background: "rgba(21,42,36,0.08)", margin: "4px 0" }}></div>
              <div style={{ padding: "6px 12px 2px", fontFamily: "var(--font-body)", fontSize: 11, color: "#A0ACA6", textTransform: "uppercase", letterSpacing: 0.06 }}>Switch account</div>
              {switches.map((s, i) => (
                <button key={i} onClick={() => { setOpen(false); s.go && s.go(); }}
                        style={{ width: "100%", display: "flex", alignItems: "center", gap: 10, padding: "10px 12px", background: "transparent", border: "none", borderRadius: 8, cursor: "pointer", textAlign: "left", fontFamily: "var(--font-body)", fontSize: 14, color: "#152A24" }}
                        onMouseOver={(e) => e.currentTarget.style.background = "#F5F5F5"}
                        onMouseOut={(e) => e.currentTarget.style.background = "transparent"}>
                  <Icon name={s.icon || "repeat"} size={16} /> {s.label}
                </button>
              ))}
            </React.Fragment>
          )}
