...
/* Row action menu — small dropdown for table row actions. */
function RowActionMenu({ actions }) {
  ...
  return (
    <div ref={ref} style={{ position: "relative", display: "inline-block" }}>
      <button onClick={() => setOpen(!open)} className="btn btn--ghost btn--sm" style={{ padding: "0 10px" }}>
        <Icon name="more-horizontal" size={16} />
      </button>
      {open && (
        <div style={{ position: "absolute", right: 0, top: "calc(100% + 4px)", minWidth: 170, background: "#fff", border: "1px solid rgba(21,42,36,0.08)", borderRadius: 10, boxShadow: "0 10px 28px -8px rgba(21,42,36,0.18)", padding: 4, zIndex: 50 }}>
          {actions.map((a, i) => {
            const danger = a.label === "Delete";
            return (
              <button key={i} onClick={() => { setOpen(false); a.onClick && a.onClick(); }}
                      style={{ width: "100%", display: "flex", alignItems: "center", gap: 8, padding: "8px 12px", background: "transparent", border: "none", borderRadius: 6, cursor: "pointer", textAlign: "left", fontFamily: "var(--font-body)", fontWeight: 500, fontSize: 13, color: danger ? "#B91C1C" : "#152A24" }}
...
window.RowActionMenu = RowActionMenu;
...
const ADMIN_COURSES_SEED = [
  ...
  { id: 4, title: "Dashboards & Storytelling",  subject: "Data Viz",         semesters: 2, lessons: 12, students: 142, status: "archived",  updated: "1 w ago" }
];

const AdminCourses = ({ user, onNav, defaultView = "list", defaultEditing = null, nav = ADMIN_NAV, notifications = ADMIN_NOTIFS, accent = "Administrator" }) => {
  ...
  const startNew = () => { if (onNav) { onNav("new"); return; } setEditing(null); setView("editor"); };
  ...
  if (view === "editor") {
    return (
      <Shell navItems={nav} active="courses" onNav={onNav} user={user} title={editing ? "Edit course" : "New course"} accent={accent} onLogout={() => onNav("logout")} notifications={notifications}>
  ...
  return (
    <Shell navItems={nav} active="courses" onNav={onNav} user={user} title="Courses" accent={accent} onLogout={() => onNav("logout")} notifications={notifications}>
      <div className="page">
        <div className="page-header">
          <div>
            <h1>Courses</h1>
        ...
        <div className="tbl-card">
          <table className="tbl">
            <thead>
              <tr>
                <th>Course</th><th>Semesters</th><th>Updated</th><th>Status</th><th style={{ textAlign: "right" }}>Action</th>
              </tr>
            </thead>
            <tbody>
              {courses.map(c => {
                const actions = c.status === "draft"
                  ? [
                      { label: "View",    icon: "eye",         onClick: () => startEdit(c) },
                      { label: "Edit",    icon: "edit-3",      onClick: () => startEdit(c) },
                      { label: "Publish", icon: "upload-cloud",onClick: () => flash({ tone: "success", title: "Course published", message: c.title }) },
                      { label: "Delete",  icon: "trash-2",     onClick: () => flash({ tone: "warning", title: "Course deleted",   message: c.title }) }
                    ]
                  : c.status === "published"
                  ? [
                      { label: "View",      icon: "eye",        onClick: () => startEdit(c) },
                      { label: "Edit",      icon: "edit-3",     onClick: () => startEdit(c) },
                      { label: "Unpublish", icon: "download-cloud", onClick: () => flash({ tone: "warning", title: "Course unpublished", message: c.title }) },
                      { label: "Archive",   icon: "archive",    onClick: () => flash({ tone: "warning", title: "Course archived",   message: c.title }) },
                      { label: "Delete",    icon: "trash-2",    onClick: () => flash({ tone: "warning", title: "Course deleted",    message: c.title }) }
                    ]
                  : [
                      { label: "View",   icon: "eye",     onClick: () => startEdit(c) },
                ...
                return (
                  <tr key={c.id}>
                    ...
                    <td className="muted">{c.semesters} sem</td>
                    ...
                    <td>
                      ...
                      {c.status === "archived"  && <Badge tone="archive">Archived</Badge>}
                    </td>
...
const SuperAdminAdmins = ({ user, onNav, nav = SUPERADMIN_NAV, notifications = SUPER_NOTIFS, accent = "Super Admin" }) => {
  ...
  return (
    <Shell navItems={nav} active="admins" onNav={onNav} user={user} title="Administrators" accent={accent} onLogout={() => onNav("logout")} notifications={notifications}>
      <div className="page">
        <div className="page-header">
          <div>
